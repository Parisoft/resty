package com.github.parisoft.resty.feature.client.execute.patch;
...
public class ExecutePatchStepDefs {
    ...
    @When("^do a execute PATCH request to \"(.*?)\" for a Response instance from json$")
    public void do_a_GET_request_to_for_a_Response_instance_from_json(String path) throws Throwable {
        actualResponse = RESTy.request(LocalServer.getHost())
                ...
                .path(path)
                .client()
                .execute("PATCH", entity);
    }

    @When("^do a execute PATCH request to \"(.*?)\" for a list of Car instances from json$")
    public void do_a_GET_request_to_for_a_list_of_Car_instances_from_json(String path) throws Throwable {
        actualCarList = RESTy.request(LocalServer.getHost())
                ...
                .path(path)
                .client()
                .execute("PATCH", entity, new TypeReference<List<Car>>() {});
    }

    @When("^do a execute PATCH request to \"(.*?)\" for a Car instance from json$")
    public void do_a_GET_request_to_for_a_Car_instance_from_json(String path) throws Throwable {
        actualCar = RESTy.request(LocalServer.getHost())
                ...
                .path(path)
                .client()
                .execute("PATCH", entity, Car.class);
    }

    @When("^do a execute PATCH request to \"(.*?)\" for a Response instance from xml$")
    ...
    @When("^do a execute PATCH request to \"(.*?)\" for a list of Car instances from xml$")
    ...
    @When("^do a execute PATCH request to \"(.*?)\" for a Car instance from xml$")
