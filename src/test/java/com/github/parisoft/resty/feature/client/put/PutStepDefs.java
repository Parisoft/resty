package com.github.parisoft.resty.feature.client.put;
...
public class PutStepDefs {
    ...
    @When("^do a PUT request to \"(.*?)\" for a Response instance from json$")
    ...
    @When("^do a PUT request to \"(.*?)\" for a list of Car instances from json$")
    public void do_a_GET_request_to_for_a_list_of_Car_instances_from_json(String path) throws Throwable {
        actualCarList = RESTy.request(LocalServer.getHost())
                ...
                .path(path)
                .client()
                .put(entity, new TypeReference<List<Car>>() {});
    }

    @When("^do a PUT request to \"(.*?)\" for a Car instance from json$")
    public void do_a_GET_request_to_for_a_Car_instance_from_json(String path) throws Throwable {
        actualCar = RESTy.request(LocalServer.getHost())
                ...
                .path(path)
                .client()
                .put(entity, Car.class);
    }

    @When("^do a PUT request to \"(.*?)\" for a Response instance from xml$")
    ...
    @When("^do a PUT request to \"(.*?)\" for a list of Car instances from xml$")
    ...
    @When("^do a PUT request to \"(.*?)\" for a Car instance from xml$")
